...
- **ai-worker 单端点**：`cd services/meridian-ai-worker && pnpm wrangler dev --port 8787`，
  curl 打 `/meridian/<endpoint>`。本目录 `wrangler` 锁在 `^3.x`（`services/meridian-ai-worker/package.json`），
  remote R2 binding 需要 `>= 4.37`——起不来就换 `apps/backend/node_modules/.bin/wrangler`
  （见 `services/meridian-ai-worker/wrangler.toml` 里的注释）。
