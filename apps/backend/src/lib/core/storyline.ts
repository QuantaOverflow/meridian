...
//
// 病灶（2026-08-30 admin-brief-1788058777778 的 cluster 47，22 条 story / 91 篇）：
// 一场大灾难被切成 22 条，去重只能压到 11 条，简报里就是 11 个块——「伤亡」占 2 块、
// 「堰塞湖」占 2 块、「国际援助」占 3 块，同一条新闻播三遍。接主线分块后 11 块 → 5 块，
// 尼泊尔占版面 44% → 26%，其他事件的块数不变（b′ 端到端实测）。
//
// ⚠️ 别引用 09-02 cluster 22 那组「丢弃 34 篇」的数字，那是错的：那 9 行里 3 行是去重回写的
// 合并组成员，文章并集只有 38 篇不是 63，真正没进简报的独有内容只有 4 篇。
//
// 这一层与去重层正交，别混：
