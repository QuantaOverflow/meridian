...
/**
 * Fetches an article using Cloudflare Browser Run (formerly Browser Rendering) via the BROWSER binding
 *
...
export async function getArticleWithBrowser(env: Env, url: string) {
  ...
  try {
    // Browser Run binding 的 quickAction：请求体字段与 REST /content 相同，返回的 Response 也是同一份 JSON
