 ...
 * 请求、X-API-Token、x-trace-id 只在 post() 里拼。本地开发由 `services/meridian-ml-service/dev-shim/` 顶替该 Worker。
...
class MLService {
  ...
  // 传输只写这一处：经 ML_SERVICE binding POST JSON，带 X-API-Token，自动注入 x-trace-id 以贯通跨 service 日志。
