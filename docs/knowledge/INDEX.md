...
- **可复用机制与方法**：[判金标能否跨架构存活：看证据的参照系是外部素材还是系统中间产物——后者随那一层退役即作废](nodes/mechanism-gold-survival-by-evidence-frame.md) · `recorded`
...
- [intel-grounding 判官选型：Claude κ=0.779 通过（离线，未过真 API 复验）；qwen 现场重跑 κ=0.407 FAIL](nodes/experiment-intel-grounding-judge-selection.md) · `recorded` · 结果：mixed · **历史摘要，运行细节不完整** · `experiment-intel-grounding-judge-selection`
  ...
  - 入边 `based_on` ← [判金标能否跨架构存活：看证据的参照系是外部素材还是系统中间产物——后者随那一层退役即作废](nodes/mechanism-gold-survival-by-evidence-frame.md)
...
- [判金标能否跨架构存活：看证据的参照系是外部素材还是系统中间产物——后者随那一层退役即作废](nodes/mechanism-gold-survival-by-evidence-frame.md) · `recorded` · `mechanism-gold-survival-by-evidence-frame`
  - 范围：判定「一份已有金标在被测系统换架构之后还能不能用」，以及建新金标时该怎么存证据；不判定金标的标注质量，也不判定它测得准不准
  - 来源性质：local_record
  - 条件：适用前提：金标的形式是 (claim, 证据) → 判定。判定只有在证据在手时才能复现，所以问题等价于「那份证据还在不在、还能不能产生」；不适用：target 直接是输入自身性质的金标（例如「这两篇文章讲不讲同一件事」），那类天然不依赖系统产物，恒久有效——它们是本判据的平凡情形；判定所需材料：打开证据快照文件看里面存的是什么。不能只看字段名或 README 声称
  - 失效条件：若出现一份金标，它的证据虽然是系统中间产物、但那一层被证明会长期稳定存在（例如冻结成契约、或有独立的兼容性保证），则二分类不够用，需要第三档「中间产物但受保护」。另一种失效：若将来能把证据重建做成确定性且零成本的（例如中间产物可由原始素材确定性复算），raw_source 与 retired_intermediate 的区别就退化成「重建贵不贵」，判据应改为按重建成本分档
  - 依据 → [intel-grounding 判官选型：Claude κ=0.779 通过（离线，未过真 API 复验）；qwen 现场重跑 κ=0.407 FAIL](nodes/experiment-intel-grounding-judge-selection.md)
