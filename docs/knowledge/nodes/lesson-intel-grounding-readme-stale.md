---
{
  ...
  "status": "recorded",
  "tasks": ["设计验收门"],
  "scope": "本仓 eval/ 下所有依赖 README 顶部「状态行」传达验收进度的 harness；不特指某一次代码缺陷",
  "source": "eval/intel-grounding/README.md:12（截至 2026-09-22 仍写「本线状态：harness ready，judge 未验证...不得把本线的任何读数当可信」）对照 commit 36e186a（2026-07-10）message 里已经给出的 κ 数字；README.md:68-77「还差什么才算 eval 真正完成」节列出的三步（建真人金标/填充合成批/跑 κ 验收闸）在 commit 时间线上其实已经做完前两步、第三步也已经跑出过结果",
  "conditions": [
    ...
    "对照组：coverage-judge 的 README 在验完 κ 后确实把「结果」小节写回了 README（见 eval/coverage-judge/README.md「结果（2026-07-03 首验）」节），状态描述与实际验收结果保持同步——同一个仓库里两种做法并存，intel-grounding 是反例，coverage-judge 是正例"
  ],
